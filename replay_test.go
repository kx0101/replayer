...
func TestRun(t *testing.T) {
	t.Run("single target success", func(t *testing.T) {
		...
		entries := []LogEntry{{Method: "GET", Path: "/", Headers: map[string][]string{}}}
		args := &CliArgs{Targets: []string{server.Listener.Addr().String()}, Concurrency: 1, Timeout: 5000}
	...
	t.Run("rate limiting", func(t *testing.T) {
		...
		args := &CliArgs{Targets: []string{server.Listener.Addr().String()}, Concurrency: 1, Timeout: 5000, RateLimit: 2}
	...
	t.Run("concurrent execution", func(t *testing.T) {
		...
		for i := range entries {
			entries[i] = LogEntry{Method: "GET", Path: "/", Headers: map[string][]string{}}
		}

		args := &CliArgs{Targets: []string{server.Listener.Addr().String()}, Concurrency: 5, Timeout: 5000}
	...
	t.Run("delay between requests", func(t *testing.T) {
		...
		args := &CliArgs{Targets: []string{server.Listener.Addr().String()}, Concurrency: 1, Timeout: 5000, Delay: 200}
...
func TestReplaySingle(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		entry := LogEntry{Method: "GET", Path: "/", Headers: map[string][]string{}}
		res := ReplaySingle(0, entry, &http.Client{Timeout: 5 * time.Second}, server.Listener.Addr().String(), &CliArgs{})
	...
	t.Run("body sent", func(t *testing.T) {
		...
		ReplaySingle(0, entry, &http.Client{Timeout: 5 * time.Second}, server.Listener.Addr().String(), &CliArgs{})
...
func TestBuildRequest(t *testing.T) {
	...
	req, err := BuildRequest(entry, "localhost:8080", &CliArgs{})
	if err != nil {
		t.Fatal(err)
	}
