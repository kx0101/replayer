...
func evaluateStatusMismatchRule(rule *StatusMismatchRule, results []models.MultiEnvResult, scope string) []RuleFailure {
	count := 0
...
func evaluateBodyDiffRule(rule *BodyDiffRule, results []models.MultiEnvResult, scope string) []RuleFailure {
	if rule.Allowed {
		return nil
	}
...
func shouldIgnoreDiff(diff *models.ResponseDiff, ignorePatterns []string) bool {
...
func evaluateLatencyRule(rule *LatencyRule, current, baseline models.LatencyStats, scope string) *RuleFailure {
...
func getLatencyMetric(stats models.LatencyStats, metric string) int64 {
	switch metric {
	case "p50":
		return stats.P50
	case "p90":
...
func filterResultsByEndpoint(results []models.MultiEnvResult, path, method string) []models.MultiEnvResult {
	var filtered []models.MultiEnvResult
...
func calculateEndpointLatency(results []models.MultiEnvResult) models.LatencyStats {
	if len(results) == 0 {
	...
	return models.CalculateLatencyStats(latencies)
}
